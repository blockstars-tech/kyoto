...
contract KyotoOld is IERC20, IERC20Metadata, Context, Ownable {
    struct LogData {
        uint256 timestamp;
        uint256 amount;
    }

    // FIXME: Later change modifeier to private
    LogData[] public _logs;
    uint256 public _previousLogIndex;
     ...
     * construction.
     */
    constructor(address _teamAddress, address _publicSaleAddress, address _reserveAddress) {
        _name = "Kyoto";
        _symbol = "KYOTO";
        ...
        // FIXME: Later raplace with pancakeswap address:        0x10ED43C718714eb63d5aA57B78B54704E256024E
        IPancakeRouter02 _pancakeswapV2Router = IPancakeRouter02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        // Create a uniswap pair for this new token
        pancakeswapV2Pair = IPancakeV2Factory(_pancakeswapV2Router.factory()).createPair(address(this), _pancakeswapV2Router.WETH());
        ...
        emit Transfer(address(0), DEAD_ADDRESS, onePercentT * 50);
    }
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        require(account != teamAddress, "TeamAddress cannot be excluded");
        require(account != DEAD_ADDRESS, "Dead address cannot be excluded");
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256[4] memory) {
        ...
        tValues[1] = calculateRedistibuteFee(tAmount); // tFee
        tValues[2] = calculateLiquidityFee(tAmount); // tLiquidity
        tValues[3] = calculateBurnFee(tAmount); // tMarketing
        tValues[0] = tAmount - tValues[1] - tValues[2] - tValues[3]; // tTrasnferAmount
        return tValues;
    }
    ...
    function _burn(uint256 tBurn, uint256 rBurn) private {
        ...
        emit Transfer(msg.sender, DEAD_ADDRESS, tBurn);
    }
    
    function calculateRedistibuteFee(uint256 _amount) private view returns (uint256) {
    ...
    function isPancakeswapV2PairAddress(address account) public view returns(bool) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount,bool takeFee) private {
        ...
        if (takeFee) {
            if (_isPancakeswapV2Pair[recipient]) {
                ...
                for (uint256 i = _previousLogIndex; i < _logs.length; i++) {
                    if((block.timestamp -_logs[i].timestamp) > 1 days) {
                        _volume -= _logs[i].amount;
                    } else {
                        _previousLogIndex = i;
                        break;
                    }
                ...
                if (tAmount > (_volume * 5) / 100) {
                    _redistributeFee = 350;
                }

                _volume += tAmount;
                _logs.push(LogData(block.timestamp, tAmount));
            }
    ...
    function calculateVolume() view public returns(uint256) {
        uint256 volume;
        for (uint256 i = _previousLogIndex; i < _logs.length; i++) {
            if(block.timestamp -_logs[i].timestamp < 24) {
                volume += _logs[i].amount;
            }
        }
        return volume;
    }
