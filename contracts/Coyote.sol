    ...
    if (takeFee) {
      ...
      if (_isPancakeswapV2Pair[recipient]) {
        if (_nextResetTimesamp >= block.timestamp) {
          ...
          userSellBalanceVolume[sender].amount = userSellBalanceVolume[sender]
            .lastUpdate <= _nextResetTimesamp - 1 days
            ? tAmount
        ...
        if (
          userSellBalanceVolume[sender].amount > (_previousVolume * 5) / 100
        ) {
